...
#include <gecode/Heuristic/incNGs.hpp>
  ...
  LReSBDSBrancher<View,n,Val,a>::choice(Space& home) { //std::cout<<"choice \n";
	  ...
	  {	
		...
		for(int i=0;i<this->x.size();i++)	
			...
			{	
				for(IntVarValues j(this->x[i]);j();++j)
				{
					 
					if(count<WatNum[i*dosize+j.val()]) 
					{	count=WatNum[i*dosize+j.val()];
						b1=i;
						b2=j.val();
					}
					else if(count!=0 && count==WatNum[i*dosize+j.val()]) 
					{
						if(this->x[i].size()<this->x[b1].size())
						{
							b1=i;
							b2=j.val();	
						}						
		...
		if(count!=0){
			...
			ch=0; 
			un_num=0;
			return new PosValChoice<Val>(*this,a,b1,b2);
		}
	  }
	  ...
	  {	
		...
		for(int i=0;i<this->x.size();i++)	
			...
			{	
				...
				{
				  int sum=0;
				  for (int s=0; s < un_num; s++) 
				  {
					
					//if(this->x.size()%2==1-unS[s])
					{  
	 
						Index_class k=(*_symmetries)(unS[s],i,j.val());
						if((k.index==i&&k.val==j.val())|| (this->x[k.index].in(k.val) && this->x[k.index].assigned()))
							sum++;
